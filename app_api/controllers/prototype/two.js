...
var ethnic = '' , country = '' , photo = '' , pValue = '' , pParam = '' , eyonId = '' , countryId = '' , userVote = {'voted' : ''};
...
'photoAddSubmit' : (req , res , next) => {	photo = new Photo(req.body);
	...
	'photoVote' : (req , res , next) => { photo = req.params.photo , voted = '';
		...
		if (req.params.photo) {
			...
			Photo.findOne({'url' : photo })
																			...
																			.exec((err , photoResult) => {
																																...
																																if (!photoResult) {
																																											return config.response(res , 404 , {'message' : 'Photo entry does not exists in the record or is not available.'}); 	}
		Photo.findVotes({'entry_slug' : photo })
																							.lean({})
																							...
																							.exec((err , voterResult) => {
																																...
																																if (!voterResult) {		var vote = new PhotoVote({'entry_slug' : photo , 'votes' : 1 , 'voters' : [req.body.author]})
		...
		if (req.params && req.params.photo) {
			...
			Photo.findOne({'url' : photo })
																			...
																			.exec((err , photoResult) => {
																													...
																													if (!photoResult) {
																																									return config.response(res , 404 , {'message' : 'Photo entry does not exists in the record or is not available.'});		}
			PhotoReply.find({'entry_slug' : photo })
																								.lean({})
