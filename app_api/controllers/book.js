var async = require('async') , config = require('../config/config') , book = '' , bValue = '' , bParam = '' , eyonId = '' , country = '' , ethnic = '' , countryId = '';

const Book = require('../models/models').Book;

const bookDelete = require('../config/deleteObject/bookObject');
...
var book = '' , bValue = '' , bParam = '' , userVote = {'voted' : false , 'newVote' : '' };
...
module.exports = {
	...
	'entryAll' : (req , res , next) => {
									...
									.select(`-_id title ethnic_group country continent region century genre slug book_owner publisher isbn page_number cover_image.location`)
	...
	'entryEthnic' : (req , res , next) => {	ethnic = req.params.ethnic.split('-').join(' ');
		...
		if (req.params && req.params.ethnic) {
							...
							async.parallel({
									...
									Book.find({'ethnic_group' : eyon._id})													.lean({})

									.hint({'ethnic_group' : 1})

									.limit(20)

									.select(`-_id title continent country century ethnic_group genre region slug book_owner publisher isbn page_number cover_image.location`)
	...
	'entryCountry' : (req , res , next) => {	country = req.params.country.split('-').join(' ');
		...
		if (req.params && req.params.country) {
							...
							async.parallel({
									...
									Book.find({'country' : country._id})															.lean({})
	...
	'entryDetail' : (req , res , next) => {	book = req.params.book;

		if (req.params && req.params.book) {
																
													Book.findOne({'slug' : book })																					.lean({})

													.populate({'path' : 'author' , 'select' : 'full_name -_id'})							.lean({})

													.populate({'path' : 'votes' , 'select' : 'voters votes -_id'})						.lean({})

													.select(`-_id title date about author continent country century ethnic_group genre region slug book_owner publisher isbn page_number cover_image.location`)
	...
	'entryAddSubmit' : (req , res , next) => {	var book = new Book(req.body) , key = book.cover_image.location.split('/').pop();

		book.save((err , entryResult) => {
																					if (err) {

																						if (req.body.cover_image.location) {

																												bookDelete.delete(req.body.cover_image.location);
	...
	'entryUpdate' : (req , res , next) => {		book = req.params.book;
																																									...
																																									.lean({})
																																														.select('_id')
																																																						.hint({'_id' : 1 })
																																																																.exec(callback);		} ,
																	'Book' : (callback) => {
																																	Book.findOne({'slug' : book })
																																																	.lean({})

													.select(`-_id title date about continent country century ethnic_group genre region slug book_owner publisher isbn page_number`)
	...
	'entryUpdateSubmit' : (req , res , next) => {	bValue = req.body , book = req.params.book , key = bValue.cover_image ? (bValue.cover_image.location ? bValue.cover_image.location.split('/').pop() : '') : '';

								if (bValue.cover_image) {

											if (!bValue.cover_image.location) {

														delete bValue.cover_image;		}		}
				...
				Book.findOne({'slug' : book })
																				...
																				.select('cover_image.location slug -_id')

																				.exec((err , entryResult) => {
								if (!entryResult) {	
																										return config.response(res , 404 , {'message' : `${modelType} entry does not exists in the record or is not available.`});	}

					if (entryResult.cover_image.location && bValue.cover_image) {
									
								Book.findOneAndUpdate({'slug' : book } , {'$set' : bValue } , {'new' : true , 'setDefaultsOnInsert' : true , 'context' : true })

								.lean({})

								.select(`-_id title date about cover_image.location author continent country century ethnic_group genre region slug book_owner publisher isbn page_number`)
								...
								.exec((err , entryUpdate) => {
																								if (err) {
																														...
																														return config.errResponse(res , 400 , err);		}
																			if (entryUpdate) {
																														if (entryResult.cover_image.location) {

																														bookDelete.delete(entryResult.cover_image.location);		}
								...
								.select(`-_id title date about cover_image.location author continent country century ethnic_group genre region slugbook_owner publisher isbn page_number`)
	...
	'entryDelete' : (req , res , next) => {	book = req.params.book;
	...
	'entryDeleteSubmit' : (req , res , next) => {	book = req.params.book;
		...
		if (req.params && req.params.book) {
				...
				Book.findOneAndDelete({'slug' : book})
																														.lean({})

																														.select(`-_id slug cover_image.location`)
													...
													async.parallel({
																										'Comment' : (callback) => {
																																										Book.deleteComments(entryResult.slug)
																																																															.exec(callback);			} ,
																										'Reply' : (callback) => {
																																										Book.deleteReplies(entryResult.slug)
																																																															.exec(callback);			} ,
																										'Vote' : (callback) => {
																																										Book.deleteVotes(entryResult.slug)
																																																															.exec(callback);			} ,
																										'commentVote' : (callback) => {
																																										Book.deleteCommentVotes(entryResult.slug)
																																																															.exec(callback);			} ,
																										'replyVote' : (callback) => {
																																										Book.deleteReplyVotes(entryResult.slug)
																																																															.exec(callback);			} ,
														} , (err , result) => {
																											if (err) {
																																						return config.errResponse(res , 400 , err);		}
	...
	'entryVote' : (req , res , next) => { book = req.params.book;
	...
	'entryAddVote' : (req , res , next) => { book = req.params.book;
		...
		if (req.params && req.params.book) {
			...
			if (entryResult) {
													Book.findVotes(book)
																								...
																								.exec((err , voterResult) => {
																														...
																														else if (!voterResult) {		var vote = new BookVote({'entry_slug' : book , 'votes' : 1 , 'voters' : [req.body.author]});
			...
			BookVote.findOneAndUpdate({'entry_slug' : entryResult.slug } , {'$addToSet' : {'voters' : req.body.author } , '$inc' : {'votes' : 1 }} , {'new' : true })
	...
	'commentVote' : (req , res , next) => { comment = req.params.comment;
		...
		if (req.params && req.params.comment) {
			...
			BookComment.findOne({'slug' : comment })
																								.lean({})
				...
				if (commentResult) {
						...
						BookCommentVote.findOne({'comment_slug' : comment })
																																		.lean({})
	...
	'commentAddVote' : (req , res , next) => { var { book , comment } = req.params;
		...
		if (req.params && req.params.book) {
			...
			BookCommentVote.findOne({'comment_slug' : comment})
																											...
																											if (!voterResult) {		var vote = new BookCommentVote({'entry_slug' : book , 'comment_slug' : comment , 'votes' : 1 , 'voters' : [req.body.author]});
		...
		BookCommentVote.findOneAndUpdate({'comment_slug' : comment } , {'$addToSet' : {'voters' : req.body.author } , '$inc' : {'votes' : 1}} , {'new' : true })
	...
	'replyVote' : (req , res , next) => { reply = req.params.reply;
		...
		if (req.params.reply) {
				...
				if (replyResult) {
						...
						BookReplyVote.findOne({'reply_slug' : reply })
																																		.lean({})
	...
	'replyAddVote' : (req , res , next) => { var { book , comment , reply } = req.params;
		...
		if (req.params && req.params.book) {
			...
			BookComment.findOne({'slug' : comment })
					...
					BookReplyVote.findOne({'reply_slug' : reply})
																		...
																		if (!voterResult) {	var vote = new BookReplyVote({'entry_slug' : book , 'comment_slug' : comment , 'reply_slug' : reply , 'votes' : 1 , 'voters' : [req.body.author]});
		...
		BookReplyVote.findOneAndUpdate({'reply_slug' : reply} , {'$addToSet' : {'voters' : req.body.author } , '$inc' : {'votes' : 1}} , {'new' : true })
	...
	'entryComment' : (req , res) => { 	book = req.params.book;
	...
	'entryCommentDetail' : (req , res , next) => { var { book , comment } = req.params;
			...
			BookComment.findOne({'slug' : comment , 'entry_slug' : entryResult.slug})
								...
								return config.response(res , 404 , {'message' : `No Book id provided. Please provide a valid book id.`});			}
		} , 

	'entryAddComment' : (req , res) => { var  book = req.params.book;
	...
	'entryAddCommentSubmit' : (req , res) => {	var comment = new BookComment(req.body) , book = req.params.book;

				comment.set('entry_slug' , book);
	...
	'entryCommentUpdate' : (req , res , next) => {  var { book , comment } = req.params;
	...
	'entryCommentUpdateSubmit' : (req , res , next) => { var { book , comment } = req.params;
		...
		BookComment.findOneAndUpdate({'slug' : comment , 'entry_slug' : entryResult.slug} , {'$set' : req.body } , {'new' : true , 'setDefaultsOnInsert' : true , 'context' : true })
									...
									.exec((err , commentResult) => {
																											...
																											if (!commentResult) {
																																							return config.response(res , 404 , {'message' : `Comment does not exists in the record or is not available for this entry.`});	}
									$commentUpdate = {	'book' : entryResult ,
	...
	'entryCommentDeleteAll' : (req , res , next) => {	book = req.params.book;
				...
				BookComment.deleteMany({'entry_slug' : book })
	...
	'entryCommentDelete' : (req , res , next) => { var { book , comment } = req.params;
		...
		if (req.params && req.params.book) {
			...
			BookComment.findOne({'slug' : comment , 'entry_slug' : entryResult.slug})
																								...
																								if (!commentResult) {
																																					return config.response(res, 404 , {'message' : `Comment does not exists in the record or is not available for this entry.`});		}
										$bookComment = {	'book' : entryResult ,

																			'comment' : commentResult		};
																																									return config.response(res , 200 , $bookComment);			});				}
				else {
								return config.response(res , 404 , {'message' : `No Comment id provided. Please provide a valid comment id.`});		}			}				});	 		}
				else {
								return config.response(res , 404 , {'message' : `No ${modelType} id provided. Please provide a valid ${modelType} id.`});			}
	...
	'entryCommentDeleteSubmit' : (req , res , next) => { var { book , comment } = req.params;
		...
		if (req.params && req.params.book) {
			...
			BookComment.findOneAndDelete({'slug' : comment , 'entry_slug' : entryResult.slug})
	...
	'entryReply' : (req , res) => { 	book = req.params.book;
	...
	'entryReplyDetail' : (req , res , next) => { var { book , reply } = req.params;
			...
			BookReply.findOne({'slug' : reply , 'entry_slug' : entryResult.slug})
																																							.lean({})
	...
	'entryAddReplytoComment' : (req , res) => { var  book = req.params.book , comment = req.params.comment;
		...
		if (req.params && req.params.book) {
			...
			BookComment.findOne({'entry_slug' : entryResult.slug , 'slug' : comment})
																																								.lean({})
	...
	'entryAddReplyToCommentSubmit' : (req , res) => {	var reply = new BookReply(req.body) , book = req.params.book , comment = req.params.comment;
	...
	'entryReplyUpdate' : (req , res , next) => { var { book , comment , reply } = req.params;
	...
	'entryReplyUpdateSubmit' : (req , res , next) => { var { book , comment , reply } = req.params;
		...
		BookReply.findOneAndUpdate({'slug' : reply , 'entry_slug' : entryResult.slug} , {'$set' : req.body } , {'new' : true , 'setDefaultsOnInsert' : true , 'context' : true })
	...
	'entryReplyDeleteAll' : (req , res , next) => {	book = req.params.book;
				...
				BookReply.deleteMany({'entry_slug' : book })
																											.lean({})
	...
	'entryReplyDelete' : (req , res , next) => { var { book , comment , reply } = req.params;
	...
	'entryReplyDeleteSubmit' : (req , res , next) => { var { book , comment , reply } = req.params;
			...
			BookReply.findOneAndDelete({'slug' : reply , 'entry_slug' : entryResult.slug})
	...
	'objectDelete' : (req , res , next) => {	book = req.params.object;

		if (req.params && req.params.object) {
																							bookDelete.objectDelete(req, res , next , book);												
			}	else {
								return config.response(res , 404 , {'message' : `No ${modelType} id provided. Please provide a valid ${modelType} id.`});		}
