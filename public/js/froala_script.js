...
$formProcess.click(function(e) {
			...
			$('.fr-element img').removeAttr('style');
				...
				if ($('.fr-element').text().length < 220 ) {			return $('ul.ajax').css('display' , 'block').append('<li> Main body should be provided and cannot be empty.</li>');         }

				if ($('.fr-element').text().length > 6000 ) {		return $('ul.ajax').css('display' , 'block').append('<li> Main body cannot be greater than 6000 characters in length.</li>');         }
...
data.title && data.century && data.ethnic_group && data.country && data.continent && data.region && !($('.fr-element').text()) < 220 && $myData.century.length < 40  && $myData.country.length < 40 && 

$myData.continent.length < 40 && $myData.region.length < 40 && $myData.ethnic_group.length < 40)  };
