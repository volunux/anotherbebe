...
var $inputFile = document.getElementById('cover_image');
...
function $pageLoad() { 
	...
	$uploadBtn.onclick = function(e) {
		...
		if (!data.cover_image && $rmethod == 'POST') {
												
			$setText('uploadError' , 'Cover Image should be provided and cannot be empty.' , 'block');
	
					return false;
		}

			$checkImageSize(data.cover_image);

			$checkImageType(data.cover_image);

var $getHash = new XMLHttpRequest();

$getHash.open('POST' , '/uploader/sign/book/' + data.cover_image.name + '/');
...
var $photoInfo = {	'filename' : data.cover_image.name ,
...
function $uploadImage(signedUrl , data) {
	...
	$myFormData.append('file' , data.cover_image);
...
$xhrUpload.upload.onabort = function(evt) {
		...
		$setText('uploadMessage' , 'Cover Image upload cancelled. You can now upload another image.' , 'inline');
...
function $removeImage(key) {
	...
	$xhrRemove.onload = function(res) {
			...
			if (res.target.status == 200) {
				...
				if ($rmethod == 'POST') {
						...
						$setText('uploadMessage' , 'Cover Image successfully deleted. You can now upload another image.' , 'inline');
	...
	$xhrRemove.open('DELETE' , '/api/book/o/' + key);
...
'country' : document.getElementById('country').value , 'region' : document.getElementById('region').value , 'genre' : document.getElementById('genre').value , 'isbn' : document.getElementById('isbn').value ,

'ethnic_group' : document.getElementById('ethnic_group').value , 'summary' : document.getElementById('summary').value , 'cover_image' : document.getElementById('cover_image').files[0] ,

'book_owner' : document.getElementById('book_owner').value , 'publisher' : document.getElementById('publisher').value , 'page_number' : document.getElementById('page_number').value , 
...
'dimension' : document.getElementById('dimension').value };
				...
				if ($myData.cover_image) {
					...
					var $photo_detail = {'location' : $objectInfo.photo.url , 'mimetype' : $myData.cover_image.type , 'size' : $myData.cover_image.size , 'encoding' : $myData.cover_image.type};
...
$myData.title && $myData.date && $myData.century && $myData.continent && $myData.country && $myData.region && $myData.genre && $myData.ethnic_group && $myData.summary &&

$myData.book_owner && $myData.publisher && $myData.isbn && $myData.page_number && $myData.title.length < 150 && $myData.date.length < 15 && $myData.century.length < 40 && $myData.genre.length < 40 && 

$myData.country.length < 40 && $myData.continent.length < 40 && $myData.region.length < 40 && $myData.summary.length < 2000 && $myData.ethnic_group.length < 40 && $myData.summary.length > 10 && 

$myData.book_owner.length < 40 && $myData.publisher.length < 40 && $myData.isbn.length < 40  && $myData.page_number.length < 40 && $myData.date.length >= 4		);  	}		}
...
$myData.book_owner.length < 40 && $myData.publisher.length < 40 && $myData.isbn.length < 40  && $myData.page_number.length < 40 && $myData.cover_image && $myData.date.length >= 4		);		}			}
